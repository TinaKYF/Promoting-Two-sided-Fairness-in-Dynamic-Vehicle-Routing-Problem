...
class CLUSTER:
    def __init__(self,officernum,nodesData):
        self.officernum=officernum
        self.nodedata=nodesData

    def cluster_hisdata(self):
        nodelist = []
        arealist = []
        nodeData = pd.read_csv('data\\2016\\monday_01_2016_datetime_location.csv')
        ...
        num_min = int(1334 / self.officernum * 0.7)#11148
        num_max = int(1334 / self.officernum * 1.3)
        ...
        for centralnode in centralnodelist:
            ...
            for i in range(len(self.nodedata)):
                nowdis = (centralnode[0] - self.nodedata['lat'][i]) ** 2 + (centralnode[1] - self.nodedata['lon'][i]) ** 2
                if nowdis < mindis:
                    mindis = nowdis
                    candicoo = [self.nodedata['lat'][i], self.nodedata['lon'][i]]
                    candimarker = self.nodedata['st_marker_id'][i]
