...
class GaClusterArea:
    ...
    def initialOfficers(self, speed, startPointMarker, officerNum):
        for i in range(officerNum):
            self.carParkMap.officers.append(Officer(i, speed, startPointMarker[i]))
    ...
    def execute(self):
        ...
        fileName = 'Fair_cluster_area_result.csv'
    ...
    def evolvePopulation(self):
        if self.freeOfficersNum == 1:
            ...
            for t in range(self.maxGen):
                ...
                time1=time.time()
                # self.localstddis(populationscopy1)
                self.localstdcap(populationscopy2)
                time2=time.time()
                # self.computerStddis(populationscopy1)
                self.computerStdcap(populationscopy2)
                time3=time.time()
                # populationscopy1.sort(key=lambda tup: tup[2])
                populationscopy2.sort(key=lambda tup: tup[2])
                print(time2-time1)
                print(time3-time2)
                # print('stddis generated')
                # Crossover
                for i in range(int(elitistNum), int(elitistNum + crossNum)):
                    newChild = self.generateChildChro(populationscopy2)
    ...
    def generateChildChro(self,population):
        ...
        parent1Chro = copy.deepcopy(population[a][0])
        parent2Chro = copy.deepcopy(population[b][0])
    ...
    def computerStdcap(self,population):
        arealistcopy = copy.deepcopy(self.area_list)
        for popuindex in range(len(population)):
            solution=population[popuindex]
            path=solution[0]
            areacaplistcopy = copy.deepcopy(self.area_cap_list)
            for eleindex in range(len(path)-1):
                # O indicate that it is an officer Id, and the following ids are nodes that will be assigned
                if 'O' in path[eleindex][0]:
                    officer = self.findFreeOfficer(path[eleindex][0])
                    if 'O' not in path[eleindex+1][0]:
                        ...
                        areacaplistcopy[self.area_dic[node.area]] += 1
            areacaprate = []
            for a, b in zip(arealistcopy, areacaplistcopy):
                if a == 0:
                    a = 1
                caprate = b / a
                areacaprate.append(caprate)
            stdarea = statistics.stdev(areacaprate)
            solution.append(stdarea)


    def localstdcap(self, population):
        arealistcopy = copy.deepcopy(self.area_list)
        for popuindex in range(len(population)):
            solution=population[popuindex]
            path=solution[0]
            ...
            for i, ele in enumerate(path):  # ele will be ['3357s', 1.2345]
                if 'O' in ele[0] or i == pathEndNodeI:
                    ...
                    nodesNum = subPathEndI - offiIndex  # number of nodes in subpath
                    if nodesNum > 1:
                        stdareamax= float("inf")
                        winner = None
                        nStartI=offiIndex+1
                        for j in range(nStartI,subPathEndI + 1):
                            pId=path[j][0]
                            areacaplistcopy=copy.deepcopy(self.area_cap_list)
                            node=self.carParkMap.findNodeByMarkerId(pId)
                            areacaplistcopy[self.area_dic[node.area]]+=1
                            areacaprate=[]
                            for a,b in zip(arealistcopy,areacaplistcopy):
                                if a==0:
